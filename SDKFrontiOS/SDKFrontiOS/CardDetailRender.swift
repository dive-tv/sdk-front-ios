...
public class CardDetailRender : NSObject, CardDetailDelegate{
    ...
    init(_sectionsData : [String:ConfigSection], _mainSectionKey : String!, _cardDetail : CardDetail, _navigationController : UINavigationController) {
        super.init();
    ...
    func createSections (_keyForSections : [String]) -> [Section] {
        ...
        for key in _keyForSections {
            let section = Section(nibName: "Section", bundle: nil, _configSection: self.sectionsData[key]!, _cardDetail: self.cardDetail)
            section.cardDelegate = self;
