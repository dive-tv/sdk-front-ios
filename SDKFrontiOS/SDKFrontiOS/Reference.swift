...
class Reference: TextModule {
    ...
    override class func validate(_ cardDetail : CardDetail) throws {
        guard let container = cardDetail.containers[ContainerContentType.Reference] , container.data.count > 0 else{
            // TODO: create error
    ...
    override func setCardDetail(_ _configModule: ConfigModule, _cardDetail: CardDetail) {
        ...
        self.headerLabel.text = "Reference".uppercased();
        ...
        if let container = self.cardDetail.containers[ContainerContentType.Reference], let textContainer = container.data.first as? TextContainerData{
